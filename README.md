...
Deployed and benchmarked on Raspberry Pi 4 (4GB), headless mode over SSH. Results logged per-frame and saved to CSV.
...
# Patch decoder to disable KV-cache before export.
# GPT-2's default use_cache=True returns past_key_values as nested tuples
# which torch.onnx.export cannot trace. Force use_cache=False.
...
model.decoder.forward = original_forward  # restore
```
...
```
12:35:13 | Encoder: 3.054s | Decoder: 8.329s | CPU: 79.4% | RAM: 95.0%
    → Caption: a man with glasses and a beard is smiling

12:36:07 | Encoder: 1.831s | Decoder: 7.831s | CPU: 76.6% | RAM: 95.1%
    → Caption: a person is looking at a cat in a suit

12:37:55 | Encoder: 3.059s | Decoder: 6.114s | CPU: 75.7% | RAM: 95.1%
    → Caption: a man is looking at a computer screen
```
...
- **Caption repetition** — greedy decoding with short max-token limit produces repetitive captions on static scenes. Sampling or beam search would improve diversity.
